...
if not API or not API.CreateBlizzardCheckButton then
    error("AuraFix: BlizzardEditMode.lua requires addon.API.CreateBlizzardCheckButton to be defined before loading this file.")
end
if not L then
    error("AuraFix: BlizzardEditMode.lua requires addon.L to be defined before loading this file.")
end
...
local DBKEY_MASTER = "EditModeShowAurafixUI";
...
function MainFrame:Init()
    ...
    Checkbox:SetLabel("Aurafix");
    Checkbox:SetDBKey(DBKEY_MASTER);

    local function Checkbox_GetTooltip()
        local header = L["Toggle Aurafix UI"];
        local desc = L["Toggle Aurafix UI Tooltip"];
        local text, name;
